...
namespace Auxiliary
{
    ...
    public abstract class GamePhase
    {
        ...
        protected internal virtual void Draw(SpriteBatch sb, Game game, float elapsedSeconds)
        {
        ...
        protected internal virtual void Update(Game game, float elapsedSeconds)
        {
            ...
            if (Root.WasKeyPressed(Microsoft.Xna.Framework.Input.Keys.Tab))
            {
                UIElement currentlyFocused = Root.GuiActiveElement;
                int count = UIElements.Count;
                if (count > 0)
                {
                    if (currentlyFocused == null)
                    {
                        UIElements[0].Activate();
                    }
                    else
                    {
                        int index = UIElements.IndexOf(currentlyFocused);
                        if (index == count - 1)
                        {
                            UIElements[0].Activate();
                        }
